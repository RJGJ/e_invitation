...
- Existing Lists: `User` (id, name, email, password, posts, createdAt, isAdmin, refreshToken), `Post`, `Tag` — the latter two are starter-project boilerplate, unrelated to this feature and not to be touched.
- Access Control Pattern in Use: all existing lists use `access: allowAll` (starter default). This spec introduces the first list with real, per-record access control in this codebase.
- No `Event` list, no image/file upload pipeline, and no relationship from `User` to any event-like list currently exists.
...
- DEFERRED (Do NOT do this yet - saved for future tickets):
  ...
  - DO NOT build an image upload/storage pipeline. `coverImageUrl` and `gallery` store URL strings only; wiring them up to actual file storage (S3, Cloudinary, etc.) is a separate ticket.
...
export interface Event {
  ...
  coverImageUrl?: string;          // text, optional (URL string only, no upload)
  gallery?: string[];              // json, optional (array of URL strings)
...
- Modify:
  - `api/schema.ts` — add the `Event` list; add `events: relationship({ ref: 'Event.author', many: true })` to the `User` list's `fields`.
...
3. Step 3: Add the `Event` list to `api/schema.ts` with all fields from Section 4, using `select` for `type` (options: `wedding`, `birthday`, `baptism`), `json` for `gallery`, `float` for `latitude`/`longitude`, and `checkbox` with `defaultValue: false` for `allowPlusOne`/`requireApproval`.
4. Step 4: Add the `author` relationship field (`ref: 'User.events'`, `many: false`), required.
5. Step 5: Implement `access.operation` and `access.filter` exactly as specified in Section 4.
6. Step 6: Add a `resolveInput` hook (or `hooks.resolveInput`) on create that sets `author` to `{ connect: { id: session.itemId } }` server-side, ignoring any client-supplied author/authorId.
7. Step 7: Add an `updatedAt` field with a `resolveInput` hook that sets it to `new Date()` on every `create`/`update` operation.
8. Step 8: Run `keystone dev` (or the equivalent Prisma migration command used in this repo) to generate and apply the migration for the new `Event` table.
9. Step 9: Write unit tests per Section 9 and run them.
10. Step 10: Commit atomically (schema change, then hooks/access control, then tests) and push.
